...
var setupCanvas = func {
	...
	mycanvas.addPlacement({"node": "Canvas", "texture": "moving-map.png"});
}
...
var last_type = type;
var last_zoom = zoom;
var lastLiveMap = getprop("hawk/displays/live-map");
var lastDay   = TRUE;
					...
					(func {# generator function
					    ...
					    if( io.stat(img_path) == nil and me.liveMap == TRUE) { # image not found, save in $FG_HOME
					          ...
					          .fail(func (r) {#print('Failed to get image ' ~ img_path ~ ' ' ~ r.status ~ ': ' ~ r.reason);
					          				tile.set("src", "Aircraft/BAE-Hawk/Models/MovingMap/emptyTile.png");
					      					tile.update();
					      					});
					    	...
					    	tile.set("src", "Aircraft/BAE-Hawk/Models/MovingMap/noiseTile.png");
					      	tile.update();
					    }
					})();
		  		}
