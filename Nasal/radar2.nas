...
var target_process = func ( target ) {
	...
	if ( t_in_range ) {
		...
		# RADAR stuff.
		# Check if mp within our radar field (hard coded 74Â°) and if detectable.
		if ( radar_able and t_range <= range_radar2 and !our_radar_standby ) {
			...
			if ( deviation_deg > -37  and  deviation_deg < 37 and radardist.radis(target, my_radarcorr) and
t_range < horizon ) {
		...
		if ( ecm_able and ecm_on and t_radar_standby == 0 ) {
			...
			if ( t_maxrange > 0  and t_range < horizon ) {
				# Test if we are in its (arbitrary 120Â°) angular coverage or if we have a carrier.
